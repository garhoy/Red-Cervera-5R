...
def build_fpn_mask_graph(rois, feature_maps, image_meta, pool_size, num_classes, train_bn=True):
    ...
    # Increase the number of filters in the deconvolutional layer
    x = KL.TimeDistributed(KL.Conv2DTranspose(256, (2, 2), strides=2, activation="relu"),
                           name="mrcnn_mask_deconv")(x)

    # Output layer for masks with the new MASK_SHAPE
    x = KL.TimeDistributed(KL.Conv2D(num_classes, (1, 1), strides=1, activation="sigmoid"),
                           name="mrcnn_mask")(x)
...
def load_image_gt(dataset, config, image_id, augmentation=None):
    ...
    #print(f"Original mask shape: {mask.shape}")  # Añadir impresión de depuración
    ...
    #print(f"Resized mask shape: {mask.shape}")  # Añadir impresión de depuración
...
class MaskRCNN(object):
    ...
    def __init__(self, mode, config, model_dir):
        ...
        # print(f"Using MASK_SHAPE in MaskRCNN init: {self.config.MASK_SHAPE}")
        #print(f"Using Configurations in MaskRCNN init: {self.config.__dict__}")
        ...
        if mode == "training":
            ...
            if config.USE_MINI_MASK:
                ...
                input_gt_masks = KL.Input(
                    ...
                    name="input_gt_masks", dtype=bool)
                # input_gt_masks = KL.Input(shape=[config.IMAGE_MIN_DIM, config.IMAGE_MAX_DIM, None], name="input_gt_masks", dtype=bool)
                #print(f"Configured input_gt_masks shape: {input_gt_masks.shape}")  # Depuración
    ...
    #     """Sets the model log directory and epoch counter based on whether a model_path is provided."""
    #     self.epoch = 0
    #     if model_path:
    ...
    #             self.epoch = int(m.group(6)) - 1 + 1  # Epoch number in file is 1-based, Keras is 0-based
    ...
    #     else:
    #         print("Log directory and checkpoint path are not set because model_path is not provided.")
    #         self.log_dir = None
    ...
    def unmold_detections(self, detections, mrcnn_mask, original_image_shape,
        ...
        # # Imprimir depuración antes de la normalización
        # print(f"Detections (before normalization): {boxes}")
        ...
        # print(f"Detections (after normalization): {boxes}")
    ...
    def detect(self, images, verbose=0):
        ...
        detections, _, _, mrcnn_mask, _, _, _ =\
            ...
            # Imprimir salidas brutas para depuración
        # print("Detections raw output:")
        # print(detections)
        # print("mrcnn_mask raw output:")
        # print(mrcnn_mask)
        ...
        for i, image in enumerate(images):
            ...
            # print(f"ROIs: {final_rois}")
            # print(f"Class IDs: {final_class_ids}")
            # print(f"Scores: {final_scores}")
            # print(f"Masks shape: {final_masks.shape}")
        return results
